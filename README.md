...
### 5. assets/
- JS и CSS не знают о Bitrix API напрямую
...
-  Не писать код «на потом»
-  Не смешивать слои (UI ↔ бизнес)
-  Не добавлять код без коммита

-  Один логический шаг — один коммит
-  Любое падение admin — немедленный rollback
-  Всегда можно вернуться к «скелету»
...
- Меняется только осознанным решением и отдельным коммитом
